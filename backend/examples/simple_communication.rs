...
//!
//! A completely standalone demo that shows the basic communication concept
//! without depending on any Pandacea library code.
...
impl Agent {
    ...
    async fn send_message(&self, message: Message, target_port: u16) -> Result<Message, Box<dyn Error>> {
        // Create target address with the provided port
        let target_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), target_port);
...
async fn main() -> Result<(), Box<dyn Error>> {
    println!("=== Simple Communication Demo ===");
    ...
    let response = requestor.send_message(message, 8082).await?;
    ...
    println!("\n=== Responder message history ===");
    let responder_messages = responder.messages.lock().await;
    for (i, msg) in responder_messages.iter().enumerate() {
        println!("{}. [{:?}] {} -> {}: {}", 
            i + 1, 
            msg.message_type, 
            msg.sender, 
